...
describe("learn registry/dispatcher integration", () => {
  ...
  it("extension registers /learn via real command registry not prompt file", async () => {
    ...
    const runner = new ExtensionRunner([ext], runtime, cwd, SessionManager.inMemory(cwd), new ModelRegistry(await AuthStorage.create(":memory:")));
  ...
  it("dispatcher executes via runner context with session-scoped isolated agentDir and no prod DB touch", async () => {
    ...
    const runtime = new ExtensionRuntime();
    const ext = await loadExtensionFromFactory(createLearnExtension, tmpCwd, new EventBus(), runtime, "learn-dispatch-isolated");
    ...
    // Cleanup
    try { fs.rmSync(isolatedAgentDir, { recursive: true, force: true }); } catch {}
  ...
  it("approve CAS gate: pending and rejected cannot approve, needs_review can", async () => {
    ...
    const c1 = svc.observeCandidate({ episodeId: "epA", sessionId: sessId, projectIdentity: pid, toolName: "t", toolCallId: "tc1", failureMessage: "fail1" });
    ...
    // Rejected path
    const c2 = svc.observeCandidate({ episodeId: "epA", sessionId: sessId, projectIdentity: pid, toolName: "t", toolCallId: "tc2", failureMessage: "fail2" });
    ...
    const c3 = svc.observeCandidate({ episodeId: "epA", sessionId: sessId, projectIdentity: pid, toolName: "t", toolCallId: "tc3", failureMessage: "fail3" });
    ...
    // @ts-ignore access private db via any
    ...
    svc2.close();
    try { fs.rmSync(tmpAgent, { recursive: true, force: true }); } catch {}
    try { fs.rmSync(tmpCwd2, { recursive: true, force: true }); } catch {}
  });
  ...
  it("bank mismatch fails closed on projectToMnemopiReal", async () => {
    ...
    const cand = svc.observeCandidate({ episodeId: "epB", sessionId: "sessB", projectIdentity: pid, toolName: "t", toolCallId: "tcBank", failureMessage: "failBank", scope: "project" });
    ...
    svc.close();
    try { fs.rmSync(tmpAgent3, { recursive: true, force: true }); } catch {}
    try { fs.rmSync(tmpCwd3, { recursive: true, force: true }); } catch {}
  });

  it("bundle/package artifact smoke: build and execute dist/cli.js invokes /learn status without prompt file", async () => {
    const pkg = JSON.parse(await fs.promises.readFile(path.join(import.meta.dir, "../package.json"), "utf8")) as { files: string[] };
    ...
    if (!fs.existsSync(cliPath)) {
      try {
        const proc = Bun.spawn(["bun", "run", "gen:bundle"], { cwd: path.join(import.meta.dir, ".."), stdout: "pipe", stderr: "pipe" });
    ...
    if (hadMd) {
      backup = learnMd + ".bak";
      try { fs.renameSync(learnMd, backup); } catch {}
    }
    try {
      ...
      if (hadMd && backup) {
        try { fs.renameSync(backup, learnMd); } catch {}
      }
    }
    ...
    const candBundle = svcBundle.observeCandidate({ episodeId: "epBundle", sessionId: "sessBundle", projectIdentity: pidBundle, toolName: "t", toolCallId: "tcBundle", failureMessage: "fail bundle", scope: "project" });
    svcBundle.close();
    ...
    const bundleStatus = await handleLearnCommand(["status"], settingsCustom as unknown as { get(key:string):unknown }, isolatedCwd, { agentDir: isolatedAgent });
    ...
    const extBundle = await loadExtensionFromFactory(createLearnExtension, isolatedCwd, new EventBus(), runtimeBundle, "bundle-status");
    const runnerBundle = new ExtensionRunner([extBundle], runtimeBundle, isolatedCwd, SessionManager.inMemory(isolatedCwd), new ModelRegistry(await AuthStorage.create(":memory:")), undefined, settingsCustom as unknown as never, undefined, undefined, isolatedAgent, () => null as unknown as never);
    runnerBundle.initialize({ sendMessage:()=>{}, sendUserMessage:()=>{}, appendEntry:()=>{}, setLabel:()=>{}, getActiveTools:()=>[], getAllTools:()=>[], setActiveTools: async()=>{}, getCommands:()=>[], setModel: async()=>false, getThinkingLevel:()=>undefined, setThinkingLevel:()=>{}, getServiceTiers:()=>({} as never), setServiceTier:()=>{}, getSessionName:()=>undefined, setSessionName: async()=>{}, } as never, { getModel:()=>undefined, isIdle:()=>true, abort:()=>{}, hasPendingMessages:()=>false, shutdown:()=>{}, getContextUsage:()=>undefined, getSystemPrompt:()=>[], compact: async()=>{}, } as never, { getContextUsage:()=>undefined, waitForIdle: async()=>{}, newSession: async()=>({cancelled:false}), branch: async()=>({cancelled:false}), navigateTree: async()=>({cancelled:false}), switchSession: async()=>({cancelled:false}), reload: async()=>{}, compact: async()=>{}, } as never, { notify:()=>{}, confirm: async()=>false, prompt: async()=>undefined, showStatus:()=>{}, showError:()=>{}, requestRender:()=>{}, hasUI:false } as never, "print");
    const cmdBundle = runnerBundle.getCommand("learn");
    expect(cmdBundle).toBeDefined();
    const notes: string[] = [];
    const ctxBundle = { ...runnerBundle.createCommandContext(), ui: { notify: (m:string)=> notes.push(m), confirm: async()=>false, prompt: async()=>undefined, hasUI:false } as unknown as never };
    ...
    checkBundle.close();
    try { fs.rmSync(isolatedAgent, { recursive:true, force:true }); } catch {}
    try { fs.rmSync(isolatedCwd, { recursive:true, force:true }); } catch {}
    ...
    const runner2 = new ExtensionRunner([ext2], runtime2, cwd, SessionManager.inMemory(cwd), new ModelRegistry(await AuthStorage.create(":memory:")));
  ...
  it("P1-1 conservative bank: retain bank is authoritative, not derived", async () => {
    ...
    const cand = svc.observeCandidate({ episodeId:"ep1", sessionId:"sess1", projectIdentity: pid, toolName:"t", toolCallId:"tc1", failureMessage:"fail", scope:"project" });
  ...
  it("P1-2 post-write verification: null/throw keeps retryable projection_pending", async () => {
    ...
    const pid = canonicalProjectIdentity(tmpCwd);
    const mk = (tc:string)=> {
      const c = svc.observeCandidate({ episodeId:"epV", sessionId:"sessV", projectIdentity: pid, toolName:"t", toolCallId:tc, failureMessage:`fail-${tc}`, scope:"project" });
      svc.recordVerifierResult(c.id, "bun test", { verified:true, summary:"ok", toolCallId:tc, expectedCommand:"bun test", failureFingerprint:c.failureDigest, projectIdentity: pid, sessionId:"sessV", episodeId:"epV" });
    ...
    const cPos = mk("tcPos");
    const bankPos = svc.getCandidate(cPos.id) ? (svc as unknown as { getCandidate:(id:string)=> { scope:string; projectIdentity:string } }).getCandidate(cPos.id) : null;
  ...
  it("P1-3 approval without Mnemopi is retryable via stageForRetry", async () => {
    ...
    const c = svcSeed.observeCandidate({ episodeId:"epR", sessionId:"sessR", projectIdentity: pid, toolName:"t", toolCallId:"tcR", failureMessage:"failR", scope:"project" });
    svcSeed.recordVerifierResult(c.id, "bun test", { verified:true, summary:"ok", toolCallId:"tcR", expectedCommand:"bun test", failureFingerprint:c.failureDigest, projectIdentity: pid, sessionId:"sessR", episodeId:"epR" });
    svcSeed.close();
    const staged = await handleLearnCommand(["approve", c.id, "reviewed content for retry"], settingsCustom as unknown as {get(key:string):unknown}, tmpCwd, { agentDir: tmp });
  ...
  it("P1-4 delete/rollback require stored bank; bankless not_found fails closed", async () => {
    ...
    const svc = new CustomAutolearnService(tmp);
    // Create and project
    const cand = svc.observeCandidate({ episodeId:"epD", sessionId:"sessD", projectIdentity: pid, toolName:"t", toolCallId:"tcD", failureMessage:"failD", scope:"project" });
    ...
    const cand2 = svc.observeCandidate({ episodeId:"epD2", sessionId:"sessD2", projectIdentity: pid, toolName:"t", toolCallId:"tcD2", failureMessage:"failD2", scope:"project" });
    svc.recordVerifierResult(cand2.id, "bun test", { verified:true, summary:"ok", toolCallId:"tcD2", expectedCommand:"bun test", failureFingerprint:cand2.failureDigest, projectIdentity: pid, sessionId:"sessD2", episodeId:"epD2" });
