...
describe("grouped fixes regression", () => {
  it("cwd via sessionManager.getCwd binds live session cwd, not session.cwd nor process.cwd", async () => {
    ...
    handler({ type: "tool_execution_end", toolName: "bash", toolCallId: "tc-cwd-1", isError: true, result: "fail-a" });
    await Bun.sleep(10);
    ...
    expect(svc.listCandidates(canonicalProjectIdentity(legacyCwd)).find(c => c.toolCallId === "tc-cwd-1")).toBeUndefined();
    // change live cwd and observe second candidate binds to new identity
    currentLive = liveCwdB;
    handler({ type: "tool_execution_end", toolName: "bash", toolCallId: "tc-cwd-2", isError: true, result: "fail-b" });
    await Bun.sleep(10);
    ...
    try { fs.rmSync(liveCwdA, { recursive: true, force: true }); } catch {}
    try { fs.rmSync(liveCwdB, { recursive: true, force: true }); } catch {}
    try { fs.rmSync(legacyCwd, { recursive: true, force: true }); } catch {}
  });
  ...
  it("bash transport with bun test proof uses proof.expectedCommand as verifier and links", async () => {
    ...
    const svc = new CustomAutolearnService(dir);
    handler({ type: "tool_execution_end", toolName: "bash", toolCallId: "tc-bash", isError: true, result: "fail bash" });
    await Bun.sleep(10);
    ...
    handler({ type: "tool_execution_start", toolName: "bash", toolCallId: "verifier-bash-bun", args: { command: "bun test" } });
    handler({ type: "tool_execution_end", toolName: "bash", toolCallId: "verifier-bash-bun", isError: false, result: proof });
  ...
  it("cap at 20 applies only to new failure candidates; verifier transitions still succeed when capped", async () => {
    ...
    // Fill 20 candidates
    for (let i = 0; i < 20; i++) {
      handler({ type: "tool_execution_end", toolName: "bash", toolCallId: `tc-cap-${i}`, isError: true, result: `fail ${i}` });
    }
    await Bun.sleep(20);
    const all20 = svc.listCandidates(proj);
    expect(all20.length).toBe(20);
    // 21st failure should be rejected due to cap
    handler({ type: "tool_execution_end", toolName: "bash", toolCallId: "tc-cap-20", isError: true, result: "fail 20" });
    await Bun.sleep(10);
    ...
    handler({ type: "tool_execution_start", toolName: "bash", toolCallId: "verifier-cap", args: { command: "bun test" } });
    handler({ type: "tool_execution_end", toolName: "bash", toolCallId: "verifier-cap", isError: false, result: proof });
  ...
  it("post-write uncertainty persists reference and retry reconciles without duplicate", async () => {
    ...
    const proj = canonicalProjectIdentity(cwd);
    const svc = new CustomAutolearnService(dir);
    const cand = svc.observeCandidate({ episodeId: "ep-retry", sessionId: "sess-retry", projectIdentity: proj, toolName: "bash", toolCallId: "tc-retry", failureMessage: "fail retry", scope: "project" });
    svc.recordVerifierResult(cand.id, "bun test", { verified: true, summary: "ok", toolCallId: "tc-retry", expectedCommand: "bun test", failureFingerprint: cand.failureDigest, projectIdentity: proj, sessionId: "sess-retry", episodeId: "ep-retry" });
    ...
    const cand2 = svc2.observeCandidate({ episodeId: "ep2", sessionId: "sess2", projectIdentity: proj, toolName: "bash", toolCallId: "tc2", failureMessage: "fail2", scope: "project" });
  ...
  it("bank mismatch preserves reference and marks needs_review without orphan", async () => {
    ...
    const proj = canonicalProjectIdentity(cwd);
    const svc = new CustomAutolearnService(dir);
    const cand = svc.observeCandidate({ episodeId: "ep-bm", sessionId: "sess-bm", projectIdentity: proj, toolName: "bash", toolCallId: "tc-bm", failureMessage: "fail bm", scope: "project" });
    svc.recordVerifierResult(cand.id, "bun test", { verified: true, summary: "ok", toolCallId: "tc-bm", expectedCommand: "bun test", failureFingerprint: cand.failureDigest, projectIdentity: proj, sessionId: "sess-bm", episodeId: "ep-bm" });
  ...
  it("bash carrying bun test with mismatched actual command does not promote (trusted start metadata)", async () => {
    ...
    const svc = new CustomAutolearnService(dir);
    handler({ type: "tool_execution_end", toolName: "bash", toolCallId: "tc-mismatch", isError: true, result: "fail mismatch" });
    await Bun.sleep(10);
    ...
    handler({ type: "tool_execution_start", toolName: "bash", toolCallId: "verifier-mismatch", args: { command: "echo hi" } });
    handler({ type: "tool_execution_end", toolName: "bash", toolCallId: "verifier-mismatch", isError: false, result: proof });
    await Bun.sleep(10);
    ...
    handler({ type: "tool_execution_end", toolName: "bash", toolCallId: "verifier-no-start", isError: false, result: proof2 });
    await Bun.sleep(10);
    expect(svc.getCandidate(cand.id)?.status).toBe("pending");
    svc.close();
    ctrl.close();
  ...
  it("approved retry reconciliation sets approved atomically (P1.2)", async () => {
    ...
    const proj = canonicalProjectIdentity(cwd);
    const svc = new CustomAutolearnService(dir);
    const cand = svc.observeCandidate({ episodeId: "ep-ar", sessionId: "sess-ar", projectIdentity: proj, toolName: "bash", toolCallId: "tc-ar", failureMessage: "fail ar", scope: "project" });
    svc.recordVerifierResult(cand.id, "bun test", { verified: true, summary: "ok", toolCallId: "tc-ar", expectedCommand: "bun test", failureFingerprint: cand.failureDigest, projectIdentity: proj, sessionId: "sess-ar", episodeId: "ep-ar" });
  ...
  it("rememberScoped undefined is needs_review not projection_pending (P1.4)", async () => {
    ...
    const proj = canonicalProjectIdentity(cwd);
    const svc = new CustomAutolearnService(dir);
    const cand = svc.observeCandidate({ episodeId: "ep-noid", sessionId: "sess-noid", projectIdentity: proj, toolName: "bash", toolCallId: "tc-noid", failureMessage: "fail noid", scope: "project" });
    svc.recordVerifierResult(cand.id, "bun test", { verified: true, summary: "ok", toolCallId: "tc-noid", expectedCommand: "bun test", failureFingerprint: cand.failureDigest, projectIdentity: proj, sessionId: "sess-noid", episodeId: "ep-noid" });
  ...
  it("durable delete intent persists before external and recovers on restart (P1.3)", async () => {
    ...
    const proj = canonicalProjectIdentity(cwd);
    const svc = new CustomAutolearnService(dir);
    const cand = svc.observeCandidate({ episodeId: "ep-del", sessionId: "sess-del", projectIdentity: proj, toolName: "bash", toolCallId: "tc-del", failureMessage: "fail del", scope: "project" });
    svc.recordVerifierResult(cand.id, "bun test", { verified: true, summary: "ok", toolCallId: "tc-del", expectedCommand: "bun test", failureFingerprint: cand.failureDigest, projectIdentity: proj, sessionId: "sess-del", episodeId: "ep-del" });
  ...
  it("repository-root identity: subdirectory maps same as root (P1.5)", async () => {
    ...
    const svc = new CustomAutolearnService(dir);
    handler({ type: "tool_execution_end", toolName: "bash", toolCallId: "tc-root", isError: true, result: "fail root" });
    await Bun.sleep(10);
