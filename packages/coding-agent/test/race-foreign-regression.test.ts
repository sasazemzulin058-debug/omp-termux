	...
	it("concurrent projection race does not delete first valid reference", async () => {
		...
		const mnemopiSecond: unknown = {
			...
			getScopedMemoryInBank: (id: string, _bank: string) =>
				id === "mem-race-1" || id === "mem-race-2" ? { bank: targetBank } : null,
		};
		...
		const cand2 = svc.observeCandidate({
			episodeId: "ep-race2",
			sessionId: "sess-race2",
			projectIdentity: proj,
			toolName: "bash",
			toolCallId: "tc-race2",
			failureMessage: "fail race2",
			scope: "project",
		});
		...
		// Seeded real projection_cleanup intent with different id must not delete valid reference; compensation preserves matching reference
		{
			...
			).run(cand2.id, "projection_cleanup", proj, "project", "mem-race-orphan", bank2, Date.now());
			db.close();
		...
		const mnemopiRecover: unknown = {
			...
			getScopedMemoryInBank: (id: string, _bank: string) =>
				id === "mem-race-orphan" ? null : id === "mem-race-A" ? { bank: bank2 } : null,
		};
...
describe("foreign-bank recovery filtering", () => {
	it("recoverOperationIntents filters foreign bank and leaves inaccessible intent untouched", async () => {
		...
		const candA = svc.observeCandidate({
			episodeId: "ep-fa",
			sessionId: "sess-fa",
			projectIdentity: projA,
			toolName: "bash",
			toolCallId: "tc-fa",
			failureMessage: "fail a",
			scope: "project",
		});
		svc.recordVerifierResult(candA.id, "bun test", {
			verified: true,
			summary: "ok",
			toolCallId: "tc-fa",
			expectedCommand: "bun test",
			failureFingerprint: candA.failureDigest,
			projectIdentity: projA,
			sessionId: "sess-fa",
			episodeId: "ep-fa",
		});
		...
		const candB = svc.observeCandidate({
			episodeId: "ep-fb",
			sessionId: "sess-fb",
			projectIdentity: projB,
			toolName: "bash",
			toolCallId: "tc-fb",
			failureMessage: "fail b",
			scope: "project",
		});
		svc.recordVerifierResult(candB.id, "bun test", {
			verified: true,
			summary: "ok",
			toolCallId: "tc-fb",
			expectedCommand: "bun test",
			failureFingerprint: candB.failureDigest,
			projectIdentity: projB,
			sessionId: "sess-fb",
			episodeId: "ep-fb",
		});
		...
		expect(svc.getOperationIntent(candB.id)?.mnemopiBank).toBe(bankB);
		expect(
			svc.getCandidate(candB.id)?.status === "pending" ||
				svc.getCandidate(candB.id)?.status === "needs_review" ||
				svc.getCandidate(candB.id)?.status === "projection_pending",
		).toBe(true);
