...
mock.module("@oh-my-pi/pi-utils", () => ({
	logger: { warn: () => {}, time: (_: string, fn?: unknown, ...args: unknown[]) => {
...
describe("reviewer fixes termux", () => {
	...
	it("learn-commands registers via real command API", async () => {
		const src = await fs.promises.readFile(path.resolve(import.meta.dir, "../src/autolearn/learn-commands.ts"), "utf8");
	...
	it("custom-controller requires structured proof and does not fabricate verified from exit success", async () => {
		const src = await fs.promises.readFile(path.resolve(import.meta.dir, "../src/autolearn/custom-controller.ts"), "utf8");
	...
	it("proof rejection: controller ignores output keyword and exit success without structured proof", async () => {
		...
		const mockSession: unknown = {
			...
			cwd,
			taskDepth: 0,
			subscribe: (fn: (e: unknown) => void) => { handler = fn; },
		};
		const svcFactory = (d: string) => new CustomAutolearnService(dir);
		const ctrl = new CustomAutolearnController({ session: mockSession as never, settings: settings as never, agentDir: dir, svcFactory: svcFactory as never });
		const svc = new CustomAutolearnService(dir);
		// Observe a candidate via isError true
		handler({ type: "tool_execution_end", toolName: "bash", toolCallId: "tc-proof-1", isError: true, result: "failure x" });
		...
		handler({ type: "tool_execution_end", toolName: "cargo test", toolCallId: "verifier-1", isError: false, result: "pass" });
		handler({ type: "tool_execution_end", toolName: "cargo test", toolCallId: "verifier-2", isError: false, result: "ok success 0 diagnostics" });
		handler({ type: "tool_execution_end", toolName: "cargo test", toolCallId: "verifier-3", isError: false, result: { content: "cargo test passed" } });
		await Bun.sleep(10);
		...
		handler({ type: "tool_execution_end", toolName: "cargo test", toolCallId: "verifier-structured", isError: false, result: proof });
		await Bun.sleep(10);
	...
	it("exact linkage: mismatched fingerprint/project/session/episode must not promote", async () => {
		...
		const settings = settingsFor("custom");
		let handler: (e: unknown) => void = () => {};
		const mockSession: unknown = { sessionId: "sess-exact", cwd, taskDepth: 0, subscribe: (fn: (e: unknown) => void) => { handler = fn; } };
		const ctrl = new CustomAutolearnController({ session: mockSession as never, settings: settings as never, agentDir: dir, svcFactory: (d: string) => new CustomAutolearnService(d) as never });
		const svc = new CustomAutolearnService(dir);
		...
		const baseProof = { verified: true, summary: "ok", toolCallId: cand.toolCallId, expectedCommand: "cargo test", failureFingerprint: cand.failureDigest, projectIdentity: proj, sessionId: "sess-exact", episodeId: epi };
		// Mismatched fingerprint
		handler({ type: "tool_execution_end", toolName: "cargo test", toolCallId: "v1", isError: false, result: { ...baseProof, failureFingerprint: "badbadbadbadbad1" } });
		await Bun.sleep(10);
		expect(svc.getCandidate(cand.id)?.status).toBe("pending");
		// Mismatched toolCallId
		handler({ type: "tool_execution_end", toolName: "cargo test", toolCallId: "v2", isError: false, result: { ...baseProof, toolCallId: "other" } });
		await Bun.sleep(10);
		expect(svc.getCandidate(cand.id)?.status).toBe("pending");
		// Mismatched project
		handler({ type: "tool_execution_end", toolName: "cargo test", toolCallId: "v3", isError: false, result: { ...baseProof, projectIdentity: otherProj } });
		await Bun.sleep(10);
		expect(svc.getCandidate(cand.id)?.status).toBe("pending");
		// Correct should promote
		handler({ type: "tool_execution_end", toolName: "cargo test", toolCallId: "v4", isError: false, result: baseProof });
		await Bun.sleep(10);
		expect(svc.getCandidate(cand.id)?.status).toBe("needs_review");
		svc.close();
		ctrl.close();
	...
	it("approve projects exact redacted reviewed content via scoped Mnemopi", async () => {
		...
		const proj = canonicalProjectIdentity(cwd);
		const svc = new CustomAutolearnService(dir);
		const cand = svc.observeCandidate({ episodeId: "ep-approve", sessionId: "s-approve", projectIdentity: proj, toolName: "bash", toolCallId: "tc-approve", failureMessage: "fail" });
		svc.recordVerifierResult(cand.id, "cargo test", { verified: true, summary: "ok", toolCallId: "tc-approve", expectedCommand: "cargo test", failureFingerprint: cand.failureDigest, projectIdentity: proj, sessionId: "s-approve", episodeId: "ep-approve" });
		svc.close();
		const remembered: { content: string; opts: unknown }[] = [];
		const mockMnemopi = {
			rememberScoped: (content: string, opts: { scope: string; source: string }) => { remembered.push({ content, opts }); return "mem_approve_1"; },
			editScopedMemory: () => ({ status: "deleted" }),
		};
		const res = await handleLearnCommand(["approve", cand.id, "Fix handles android bionic pidfd fallback with token ghp_1234567890123456789012345678901234567890"], settingsFor("custom"), cwd, { agentDir: dir, mnemopi: mockMnemopi as unknown as never });
	...
	it("delete/rollback preserve projection on uncertain backend failure", async () => {
		...
		const proj = canonicalProjectIdentity(cwd);
		const svc = new CustomAutolearnService(dir);
		const cand = svc.observeCandidate({ episodeId: "ep-pres", sessionId: "s-pres", projectIdentity: proj, toolName: "bash", toolCallId: "tc-pres", failureMessage: "fail" });
		svc.recordVerifierResult(cand.id, "cargo test", { verified: true, summary: "ok", toolCallId: "tc-pres", expectedCommand: "cargo test", failureFingerprint: cand.failureDigest, projectIdentity: proj, sessionId: "s-pres", episodeId: "ep-pres" });
		svc.approveCandidate(cand.id, "Real fix for preserve test", proj);
		svc.projectToMnemopi(cand.id, "mem_pres");
		svc.close();
		const failingMnemopi = { rememberScoped: () => "x" as string | undefined, editScopedMemory: () => { throw new Error("backend down"); } };
		const delRes = await handleLearnCommand(["delete", cand.id], settingsFor("custom"), cwd, { agentDir: dir, mnemopi: failingMnemopi as unknown as never });
		...
		const rollRes = await handleLearnCommand(["rollback", cand.id], settingsFor("custom"), cwd, { agentDir: dir, mnemopi: failingMnemopi as unknown as never });
	...
	it("delete without mnemopi for projected candidate is blocked and preserves", async () => {
		...
		const proj = canonicalProjectIdentity(cwd);
		const svc = new CustomAutolearnService(dir);
		const cand = svc.observeCandidate({ episodeId: "ep-del", sessionId: "s-del", projectIdentity: proj, toolName: "bash", toolCallId: "tc-del", failureMessage: "fail" });
		svc.recordVerifierResult(cand.id, "bun test", { verified: true, summary: "ok", toolCallId: "tc-del", expectedCommand: "bun test", failureFingerprint: cand.failureDigest, projectIdentity: proj, sessionId: "s-del", episodeId: "ep-del" });
