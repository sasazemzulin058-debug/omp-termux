...
describe("Mnemopi production state idempotent + exact-bank", () => {
	...
	it("duplicate idempotent write returns same id and writes once", () => {
		...
		const cnt = state.memory.beam.db
			.query("SELECT COUNT(*) as c FROM working_memory WHERE source = ?")
			.get(source) as { c: number };
		expect(cnt.c).toBe(2);
		// Verify idempotency_key column persisted
		const row = state.memory.beam.db
			.query("SELECT idempotency_key as k FROM working_memory WHERE id = ?")
			.get(id1!) as { k: string | null };
	...
	it("exact-bank same-ID collision does not cross-contaminate", () => {
		...
		const memGlobal = (
			state as unknown as { scoped: { global?: { memory: import("@oh-my-pi/pi-mnemopi").Mnemopi } } }
		).scoped.global?.memory;
		...
		const insert = (m: import("@oh-my-pi/pi-mnemopi").Mnemopi, content: string) => {
			m.beam.db.run(
				"INSERT OR REPLACE INTO working_memory (id, content, source, timestamp, session_id, importance, metadata_json, scope) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
				[id, content, "test", new Date().toISOString(), m.sessionId, 0.5, JSON.stringify({}), "bank"],
			);
		};
	...
	it("delete/rollback correct bank via exact-bank edit", () => {
		...
		mem.beam.db.run(
			"INSERT INTO episodic_memory (id, content, source, timestamp, session_id, importance, metadata_json, scope) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
			[id!, "episodic content", "test", new Date().toISOString(), mem.sessionId, 0.5, JSON.stringify({}), "bank"],
		);
	...
	it("targetBank param writes to exact bank and not to retain", () => {
		...
		const id = state.rememberScopedIdempotent(content, {
			scope: "bank",
			source: "custom-autolearn",
			idempotencyKey: key,
			targetBank: otherBank,
		});
		expect(id).toBeTruthy();
