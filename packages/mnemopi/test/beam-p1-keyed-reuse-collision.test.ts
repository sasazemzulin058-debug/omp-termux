...
describe("P1 finding 1: keyed content reuse persists key atomically across restart", () => {
	it("persists idempotency_key on content-duplicate reuse and dedupes across restart", () => {
		...
		// Verify no key yet
		const rowBefore = state.db.query("SELECT idempotency_key as k, metadata_json as m FROM working_memory WHERE id = ?").get(idNoKey) as { k: string | null; m: string | null };
		...
		const rowAfter = state.db.query("SELECT id, idempotency_key as k, metadata_json as m, source as s FROM working_memory WHERE id = ?").get(idNoKey) as { k: string | null; m: string | null; s: string };
		...
		const state2: BeamMemoryState = {
			...
			caches: {},
			pluginManager: undefined as never,
			config: { workingMemoryLimit: 1000, workingMemoryTtlHours: 1000, proactiveLinking: false } as unknown as BeamMemoryState["config"],
		} as BeamMemoryState;
		states.push(state2);

		// Deterministic keyed write after restart should return same id without inserting duplicate
		const idAfterRestart = remember(state2, content + " different payload but same key should still dedup by key", { source, idempotencyKey: key });
		expect(idAfterRestart).toBe(idNoKey);

		const countKey = state2.db.query("SELECT COUNT(*) as c FROM working_memory WHERE source = ? AND idempotency_key = ?").get(source, key) as { c: number };
	...
	it("bypasses content dedup when existing duplicate already has different key", () => {
		...
		const c1 = state.db.query("SELECT COUNT(*) as c FROM working_memory WHERE source = ? AND idempotency_key = ?").get(source, key1) as { c: number };
		const c2 = state.db.query("SELECT COUNT(*) as c FROM working_memory WHERE source = ? AND idempotency_key = ?").get(source, key2) as { c: number };
		expect(c1.c).toBe(1);
		expect(c2.c).toBe(1);
...
describe("P1 finding 2: migration repoint unique collision rollbacks", () => {
	it("merges graph_edges duplicates and completes migration without rollback", () => {
		...
		raw.run("INSERT INTO working_memory (id, content, source, timestamp, session_id, idempotency_key) VALUES (?, ?, ?, ?, ?, ?)", ["dup-keep", "content keep", source, new Date().toISOString(), "sess-x", key]);
		raw.run("INSERT INTO working_memory (id, content, source, timestamp, session_id, idempotency_key) VALUES (?, ?, ?, ?, ?, ?)", ["dup-2", "content dup2", source, new Date().toISOString(), "sess-x", key]);
		...
		const edge = db2.query("SELECT source, target, edge_type FROM graph_edges").get() as { source: string; target: string; edge_type: string };
		expect(edge.source).toBe("dup-keep");
		expect(edge.target).toBe("target-X");
		// Unique index must now exist
		const idx = db2.query("SELECT sql FROM sqlite_master WHERE type='index' AND name='idx_wm_source_idempotency_unique'").get() as { sql: string } | null;
	...
	it("merges annotation UNIQUE collision before repoint, preserving keep row and removing duplicate only after links safe", () => {
		const tmp = path.join(os.tmpdir(), `beam-p1-annot-collision-${Date.now()}-${Math.random().toString(36).slice(2)}.db`);
		files.push(tmp);

		// Raw DB without initBeam: create working_memory + annotations with UNIQUE(memory_id,kind,value)
		const raw = new Database(tmp);
		raw.run("CREATE TABLE IF NOT EXISTS working_memory (id TEXT PRIMARY KEY, content TEXT NOT NULL, source TEXT, timestamp TEXT, session_id TEXT DEFAULT 'default', importance REAL DEFAULT 0.5, metadata_json TEXT, idempotency_key TEXT DEFAULT NULL)");
		raw.run("CREATE TABLE IF NOT EXISTS annotations (id INTEGER PRIMARY KEY AUTOINCREMENT, memory_id TEXT NOT NULL, kind TEXT NOT NULL, value TEXT NOT NULL, source TEXT, confidence REAL DEFAULT 1.0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)");
		...
		raw.run("CREATE TABLE IF NOT EXISTS memory_embeddings (memory_id TEXT PRIMARY KEY, embedding_json TEXT NOT NULL, model TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)");
		...
		raw.run("INSERT INTO working_memory (id, content, source, timestamp, session_id, idempotency_key) VALUES (?, ?, ?, ?, ?, ?)", ["keep-annot", "keep content", source, new Date().toISOString(), "sess-annot", key]);
		raw.run("INSERT INTO working_memory (id, content, source, timestamp, session_id, idempotency_key) VALUES (?, ?, ?, ?, ?, ?)", ["dup-annot", "dup content", source, new Date().toISOString(), "sess-annot", key]);
		// Both memories share same annotation (kind,value) which would violate UNIQUE after repoint if not merged
		raw.run("INSERT INTO annotations (memory_id, kind, value, source) VALUES (?, ?, ?, ?)", ["keep-annot", "mentions", "Alice", "extractor"]);
		raw.run("INSERT INTO annotations (memory_id, kind, value, source) VALUES (?, ?, ?, ?)", ["dup-annot", "mentions", "Alice", "extractor"]);
		// Dup also has a distinct annotation that should be repointed
		raw.run("INSERT INTO annotations (memory_id, kind, value, source) VALUES (?, ?, ?, ?)", ["dup-annot", "mentions", "Bob", "extractor"]);
		raw.close();
		...
		const annots = db2.query("SELECT memory_id, kind, value FROM annotations ORDER BY value").all() as Array<{ memory_id: string; kind: string; value: string }>;
		...
		const dupAnnotCnt = db2.query("SELECT COUNT(*) as c FROM annotations WHERE memory_id = ?").get("dup-annot") as { c: number };
