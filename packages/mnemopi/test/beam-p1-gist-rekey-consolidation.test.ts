...
describe("P1 finding: gist identity rekey with collision preserves metadata and edges", () => {
	it("rekeys gist_duplicate to gist_kept and merges collision preserving keep metadata plus union participants, no dangling edges", () => {
		...
		raw.run(
			...
			["keep-mem", "keep content", source, new Date().toISOString(), "sess-gist", key],
		);
		raw.run(
			"INSERT INTO working_memory (id, content, source, timestamp, session_id, idempotency_key) VALUES (?, ?, ?, ?, ?, ?)",
			["dup-mem", "dup content", source, new Date().toISOString(), "sess-gist", key],
		);
		// keep gist with location Moscow, participants Alice
		raw.run(
			"INSERT INTO gists (id, text, timestamp, participants_json, location, emotion, time_scope, memory_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
			[
				"gist_keep-mem",
				"keep gist text",
		...
		raw.run("INSERT INTO graph_edges (source, target, edge_type) VALUES (?, ?, ?)", [
			"gist_keep-mem",
			"entity_X",
			"ctx",
		]);
		...
		const gists = db2
			.query("SELECT id, memory_id, text, participants_json, location FROM gists ORDER BY id")
			.all() as Array<{
			id: string;
			memory_id: string;
			text: string;
		...
		const edgeSources = db2
			.query("SELECT source, target, edge_type FROM graph_edges ORDER BY target")
	...
	it("rekeys gist when keep has no gist, creating gist_kept from duplicate without collision", () => {
		...
		raw.run(
			"INSERT INTO working_memory (id, content, source, timestamp, session_id, idempotency_key) VALUES (?, ?, ?, ?, ?, ?)",
			["keep2", "keep2 content", source, new Date().toISOString(), "sess-gist", key],
		);
		raw.run(
			"INSERT INTO working_memory (id, content, source, timestamp, session_id, idempotency_key) VALUES (?, ?, ?, ?, ?, ?)",
			["dup2", "dup2 content", source, new Date().toISOString(), "sess-gist", key],
		);
		raw.run(
			"INSERT INTO gists (id, text, timestamp, participants_json, location, emotion, time_scope, memory_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
			[
		...
		const db2 = openDatabase(tmp);
		initBeam(db2);
		const gists = db2.query("SELECT id, memory_id, text, location FROM gists ORDER BY id").all() as Array<{
			id: string;
			memory_id: string;
			text: string;
			location: string | null;
		...
		const edge = db2.query("SELECT source, target FROM graph_edges WHERE target = ?").get("entity_Z") as {
			source: string;
			target: string;
		} | null;
		expect(edge?.source).toBe("gist_keep2");
		const staleEdge = db2.query("SELECT COUNT(*) as c FROM graph_edges WHERE source = ?").get("gist_dup2") as {
			c: number;
		};
...
describe("P1 finding: consolidation exact-once with keyed uniqueness reread", () => {
	it("second consolidate with same source+key returns existing id and does not create unkeyed duplicate", () => {
		...
		db.run(
			...
			[
				wmId,
				"working content for consolidation",
				source,
				new Date().toISOString(),
				"sess-consol",
				0.7,
		...
		const summaryA =
			"Consolidated summary for idempotent source A with sufficient length to generate distinct content and trigger episodic insertion.";
		const id1 = consolidateToEpisodic(beam, summaryA, [wmId], source, 0.6, {});
		expect(typeof id1).toBe("string");
		const count1 = db
			.query("SELECT COUNT(*) as c FROM episodic_memory WHERE source = ? AND idempotency_key = ?")
			.get(source, key) as { c: number };
		expect(count1.c).toBe(1);
		const row1 = db
			.query("SELECT id FROM episodic_memory WHERE source = ? AND idempotency_key = ?")
			.get(source, key) as { id: string };
		...
		const summaryB =
			"Different consolidated summary B for same idempotent source should not create duplicate episodic row, must return existing.";
	...
	it("consolidation fallback only on missing column, not on arbitrary error creating null-keyed duplicate", () => {
		...
		const rows = db
			.query("SELECT id, idempotency_key, content FROM episodic_memory WHERE source = ?")
			.all(source) as Array<{ id: string; idempotency_key: string | null; content: string }>;
