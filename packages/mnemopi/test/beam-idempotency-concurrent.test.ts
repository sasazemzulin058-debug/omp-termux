...
describe("beam idempotency concurrent and unique constraint", () => {
	it("concurrent duplicate source+key via shared file returns same id and single row (race-safe via unique partial index)", () => {
		...
		const idxWm = stateA.db.query("SELECT name, sql FROM sqlite_master WHERE type='index' AND name='idx_wm_source_idempotency_unique'").get() as { name: string; sql: string } | null;
		expect(idxWm).not.toBeNull();
		expect(idxWm?.sql).toContain("UNIQUE");
		expect(idxWm?.sql).toContain("WHERE");
		const idxEm = stateA.db.query("SELECT name, sql FROM sqlite_master WHERE type='index' AND name='idx_em_source_idempotency_unique'").get() as { name: string; sql: string } | null;
		...
		const idB = remember(stateB, content + " different payload but same key should still dedup", { source, idempotencyKey: key });
		expect(idB).toBe(idA);
		...
		const rowCount = stateA.db.query("SELECT COUNT(*) as c FROM working_memory WHERE source = ? AND idempotency_key = ?").get(source, key) as { c: number };
		...
		let threw = false;
		try {
			stateA.db.run("INSERT INTO working_memory (id, content, source, timestamp, session_id, importance, metadata_json, idempotency_key) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", ["raw-dup-id", content, source, new Date().toISOString(), "sess-a", 0.5, null, key]);
		} catch (e) {
			threw = true;
	...
	it("migration reconciles legacy duplicates deterministically and reports", () => {
		...
		raw.run("INSERT INTO working_memory (id, content, source, timestamp, session_id, idempotency_key) VALUES (?, ?, ?, ?, ?, ?)", ["dup-1", "content dup", "custom-autolearn", new Date().toISOString(), "sess-dup", "legacy-dup-key"]);
		raw.run("INSERT INTO working_memory (id, content, source, timestamp, session_id, idempotency_key) VALUES (?, ?, ?, ?, ?, ?)", ["dup-2", "content dup 2", "custom-autolearn", new Date().toISOString(), "sess-dup", "legacy-dup-key"]);
		raw.close();
		...
		console.warn = (...args: unknown[]) => { if (String(args[0]).includes("idempotency duplicates")) warned = true; origWarn(...(args as never[])); };
		try {
			initBeam(db2);
		} finally {
		...
		const count = db2.query("SELECT COUNT(*) as c FROM working_memory WHERE source = ? AND idempotency_key = ?").get("custom-autolearn", "legacy-dup-key") as { c: number };
		expect(count.c).toBe(1);
		const remaining = db2.query("SELECT id FROM working_memory WHERE source = ? AND idempotency_key = ?").get("custom-autolearn", "legacy-dup-key") as { id: string };
